...
const columns = computed(() => {
  return [
    { title: 'Potential', items: props.data.filter(d => d.customer?.current_conversion === 'Potential').slice(0, 5), color: 'border-amber-500', bg: 'bg-amber-50' },
    { title: 'Prospect', items: props.data.filter(d => d.customer?.current_conversion === 'Prospect').slice(0, 5), color: 'border-orange-500', bg: 'bg-orange-50' },
    { title: 'Hot Prospect', items: props.data.filter(d => d.customer?.current_conversion === 'Hot Prospect').slice(0, 5), color: 'border-red-500', bg: 'bg-red-50' },
    { title: 'Purchased', items: props.data.filter(d => d.customer?.current_conversion === 'Purchased' || d.customer?.customer_status === 'Purchased').slice(0, 5), color: 'border-emerald-500', bg: 'bg-emerald-50' }
  ]
})
</script>
        ...
        <div class="flex-1 p-3 overflow-y-auto space-y-3">
          <div v-for="item in col.items" :key="item.id" class="bg-white p-3 rounded-lg shadow-sm border border-gray-200 cursor-move hover:border-primary-300 hover:shadow-md transition-all">
            <div class="flex justify-between items-start mb-2">
              <h4 class="font-bold text-sm text-gray-900">{{ item.customer?.full_name }}</h4>
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="text-[10px] text-gray-500 flex items-center">
                ...
                {{ item.employee?.full_name?.split(' ')[0] || 'Unknown' }}
              </span>
            </div>
          </div>
